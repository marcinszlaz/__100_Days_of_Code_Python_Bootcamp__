...
class NotificationManager:
  ...
  def send_email(self,subject: str,message: str):
    """sending an email"""
    try:
      with smtplib.SMTP(host=os.getenv('SMTP'), port=os.getenv('MY_PORT')) as connect:
        time_now = f"{dt.datetime.now().strftime('%H:%M:%S')}"
        connect.starttls()
        connect.login(user=os.getenv('MY_EMAIL'), password=os.getenv('MY_PASSWORD'))
        connect.sendmail(from_addr=os.getenv('MY_EMAIL'), to_addrs=[os.getenv('MY_EMAIL')], msg=f"""Subject: {subject}\n\n{message}""".encode("utf-8"))
