...
def home():
  ...
  if request.method == 'GET' or 'POST':
    with app.app_context():
      show_books = db.session.execute(db.select(Books.id,Books.author, Books.review)).tuples().all()
  try:
    if request.method == 'POST':
      ...
      with app.app_context():  # writing a row, create new record
        db.create_all()
        try:
          books_ = Books(title=title, author=author, review=review)
...
@app.route('/edit/<int:id_>', methods=['GET'])
def edit(id_):
  global edited_book_id
  with app.app_context():
    title_and_review = db.session.execute(db.select(Books.title, Books.review).where(Books.id == id_)).tuples().all()
  return render_template('edit.html', id_ = id_, title_and_review=title_and_review)
